class JeuDeLaViePersonnalise : public JeuDeLaVieBase {
public:
    JeuDeLaViePersonnalise(int lignes, int colonnes) : JeuDeLaVieBase(lignes, colonnes) {}
    ...
    void prochaineGeneration() override {
        ...
        for (int i = 0; i < lignes; ++i) {
            for (int j = 0; j < colonnes; ++j) {
                ...
                if (grille[i][j] == 1) {
                    if (voisinsVivants == 2 || voisinsVivants == 4) {
